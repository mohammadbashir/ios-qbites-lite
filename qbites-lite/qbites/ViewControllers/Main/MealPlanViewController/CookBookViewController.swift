...
//  qbites
//
//  Created by Mohammad Bashir sidani on 12/23/19.
...
//        label.numberOfLines = 0
        ...
        label.text = appParent().name + "’s Cookbook"
...
//        Utilities.sharedInstance.applyBlueGradient(view: self.view)
        ...
        GlobalCookBookViewController = self
...
//        titleLabel.topAnchor.constraint(equalTo: addRecipeButton.bottomAnchor, constant: 15).isActive = true
        ...
        mealPlanTableView.topAnchor.constraint(equalTo: titleLabel.bottomAnchor, constant: 10).isActive = true
...
//
//        self.title = appParent().name + "’s Cookbook"
...
//    }
    ...
    @objc func fetchMeals(){
        ...
        RequestHelper.sharedInstance.getCookBook { (response) in
            if(response["success"] as? Bool == true){
                SVProgressHUD.dismiss()
                
                self.meals = response["favorites"] as! [Meal]
                self.mealPlanTableView.reloadData()
